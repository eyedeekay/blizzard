...
func Main(args map[string]*string) {
	...
	if val, ok := args["stunURL"]; ok {
		stunURL = *val
	}
	if val, ok := args["logFilename"]; ok {
		logFilename = *val
	}
	if val, ok := args["rawBrokerURL"]; ok {
		rawBrokerURL = *val
	}
	if val, ok := args["relayURL"]; ok {
		RelayURL = *val
	}
	if val, ok := args["unsafeLogging"]; ok {
		var err error
		unsafeLogging, err = strconv.ParseBool(*val)
		if err != nil {
			unsafeLogging = false
		}
	}
	if val, ok := args["keepLocalAddresses"]; ok {
		var err error
		keepLocalAddresses, err = strconv.ParseBool(*val)
		if err != nil {
			keepLocalAddresses = false
		}
	}
